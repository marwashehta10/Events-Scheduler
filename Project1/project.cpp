...
void user::add(unordered_map<string, event> &tmp, stack<event> &ptr)
{
	...
	while (true)
	{
		...
		if ((s.startdate.day > 31) || (s.startdate.month > 12) || (a.wYear > s.startdate.year) || (a.wYear == s.startdate.year && a.wMonth > s.startdate.month) || (a.wYear == s.startdate.year && a.wMonth == s.startdate.month &&a.wDay > s.startdate.day))
		{
		 ...
		 if ((s.enddate.day > 31) || (s.enddate.month > 12) || (s.startdate.year > s.enddate.year) || (s.startdate.year == s.enddate.year && s.startdate.month > s.enddate.month) || (s.startdate.year == s.enddate.year && s.startdate.month == s.enddate.month && s.startdate.day > s.startdate.day))
		{
	   ...
	   if (tmp.find(s.name) != tmp.end())
		...
		if (x != 1){
			...
			{
				if (s.place == it->second.place || (s.place == it->second.place || toupper(s.place[1]) == toupper(it->second.place[1])))
				{
